...
final class JsonReader {
    public static func path(for name: String) -> String? {
        ...
        var path: String?
        if let bundleURL = podBundle.url(forResource: "SimpleCircleMenu", withExtension: "bundle") {
            if let bundle = Bundle(url: bundleURL) {
                if let file = bundle.url(forResource: name, withExtension: "json") {
                    path = file.path
                }
            }
        }
    ...
    public static func readJson(fileName: String) -> Any? {
        do {
            let podBundle = Bundle(for: JsonReader.self)
            if let bundleURL = podBundle.url(forResource: Bundle.main.bundleIdentifier, withExtension: "bundle") {
                if let bundle = Bundle(url: bundleURL) {
