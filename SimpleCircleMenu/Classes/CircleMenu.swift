...
public class CircleMenu: UIView {
    ...
    public var closeByTapOutside: Bool = false
    ...
    public func attach(to viewController: UIViewController) {
        ...
        let tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(rootTapAction))
        rootView?.addGestureRecognizer(tapRecognizer)
    ...
    private weak var lastInvokedButton: CircleMenuButton?

    @objc private func rootTapAction(recognizer: UIPanGestureRecognizer) {
        guard closeByTapOutside else { return }
        if mainButton?.isOpen ?? false {
            shadowView?.isHidden = true

            if isSubmenuOpen, let invokedButton = lastInvokedButton {
                invokedButton.onTouchUpInside()
                prepareSubmenuIfNeeded(invokedButton: invokedButton)
            }

            closeMenu()
            mainButton?.playAnimation(type: .close)
        }
    }

    @objc private func menuButtonTouchUpInside(_ button: CircleMenuButton) {
        guard let id = button.model?.id else { return }
        lastInvokedButton = button
        delegate?.menuItemSelected(id: id)
        ...
        return Promise { seal in
            ...
            for j in 0..<menuButtons.count {
                ...
                if callbackSetted {
                    ...
                    startMoveButtonAnimation(button: menuButtons[j], buttonPosition: buttonPositions[previousPositionIndex], indicatorPosition: indicatorPositions[previousPositionIndex], duration: menuOpenButtonAnimationDuration, completion: {_ in seal.fulfill(Void())})
                    callbackSetted = true
                }
            }
        }
    ...
    private func moveLeftAnimation() -> Promise<Void> {
        return Promise { seal in
            ...
            for i in 0..<menuButtons.count {
                ...
                startMoveButtonAnimation(button: menuButtons[i], buttonPosition: buttonPositions[nextPositionIndex], indicatorPosition: indicatorPositions[nextPositionIndex], duration: buttonMovementAnimationDuration, completion: {_ in seal.fulfill(Void())})
            }
        }
    }
    ...
    private func moveRightAnimation() -> Promise<Void> {
        return Promise { seal in
            ...
            for i  in 0..<menuButtons.count {
                ...
                startMoveButtonAnimation(button: menuButtons[i], buttonPosition: buttonPositions[previousPositionIndex], indicatorPosition: indicatorPositions[previousPositionIndex], duration: buttonMovementAnimationDuration, completion: {_ in seal.fulfill(Void())})
            }
        }
    }
    ...
    private func prepareSubmenuIfNeeded(invokedButton: CircleMenuButton) {
        guard invokedButton.model?.hasChildren ?? false,
            let children = invokedButton.model?.children else {
            return
        }
        precondition(children.count <= maximumCountOfChildren, "Submenu should contain no more then \(maximumCountOfChildren) elements")
        ...
        if !isSubmenuOpen {
            sendViewToBack()
            let submenuButtons = prepareSubmenu(invokedButton: invokedButton, children: children)
    ...
    private func openSubmenu(invokedButton: CircleMenuButton, submenuButtons: [CircleMenuButton]) -> Promise<Void> {
        return Promise { seal in
            ...
            guard let lastButtonFrame = submenuButtons.last?.frame else { return }
    ...
    private func closeSubmenu(invokedButton: CircleMenuButton) -> Promise<Void> {
        return Promise { seal in
            ...
            guard submenuButtons.count > 0 else {
                return
            }
            invokedButton.indicator?.isHidden = true

            var promise = startChangeAlphaAnimation(button: submenuButtons[0] as! CircleMenuButton, alpha: 0, duration: showSubmenuDuration)
            for i in 1..<submenuButtons.count {
                promise = promise.then { _ in
                    self.startChangeAlphaAnimation(button: submenuButtons[i] as! CircleMenuButton, alpha: 0, duration: self.showSubmenuDuration)
                }
            }
            promise.done {
                invokedButton.indicator?.resetPosition()
                invokedButton.indicator?.isHidden = false
                submenuButtons.forEach { button in
                    button.removeFromSuperview()
                    (button as! CircleMenuButton).removeTarget(self, action: #selector(self.onSubmenuTouchUpIside), for: .touchUpInside)
                }
                invokedButton.isUserInteractionEnabled = true
                seal.fulfill(Void())
                }.cauterize()
    ...
    private func startHintAnimationIfNeeded() -> Promise<Void> {
        return Promise { seal in
            guard let invokedButton = menuButtons.first(where: {$0.positionIndex == 1}) else {
                return
            }
            guard isHintShown == false, invokedButton.model?.hasChildren ?? false else {
                seal.fulfill(Void())
                return
            }
